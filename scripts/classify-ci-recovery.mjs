...
export function classifyRecovery(input) {
  assert.ok(["Validate", "Security"].includes(input.workflow), "Unsupported recovery workflow");
  ...
  assert.equal(input.attempt, 1, "Only the first workflow attempt is eligible for recovery");
  ...
  return {
    ...
    signals,
    action: eligible ? "rerun_failed_jobs" : "human_handoff",
  };
}
