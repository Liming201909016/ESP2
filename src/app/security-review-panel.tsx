...
export function SecurityReviewPanel({ onOpenAudit, initialId, initialQuery }: { onOpenAudit: (id: string) => void; initialId?: string | null; initialQuery?: string }) {
  ...
  const [reason, setReason] = useState(""); const [previousReviewId, setPreviousReviewId] = useState<string | null>(null);
  ...
  async function load(next?: string, signal?: AbortSignal) {
    const response = await fetch(`/api/security-reviews${next ? `?cursor=${encodeURIComponent(next)}` : ""}`, { cache: "no-store", signal });
    if (!response.ok) throw new Error("LOAD");
    const value = await response.json(); const entries = storedSecurityReviewSchema.array().parse(value.records);
    setRecords((current) => next ? [...current, ...entries] : entries); setCursor(value.nextCursor);
  }
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const catalog = await response.json(); setBackend(catalog.backend === "memory" ? "memory" : catalog.backend === "blob" ? "blob" : null);
        ...
        if (initialId) {
          ...
          const body = await detail.json(); setSelected(storedSecurityReviewSchema.parse({ record: body.record, etag: body.etag }));
        }
      } catch { if (!controller.signal.aborted) setError("reviewError_LOAD"); }
  ...
  async function action(command: Record<string, unknown>) {
    ...
    try {
      ...
      const body = await response.json(); setAudit(readAuditReceipt(body));
      ...
      setSelected(entry); setReason(""); await load();
    } catch { setError("reviewError_UNCERTAIN"); }
    finally { busy.current = false; setPending(false); }
  }
  async function open(id: string) {
    if (busy.current) return; busy.current = true; setPending(true); setError(null);
    try {
      const response = await fetch(`/api/security-reviews?id=${encodeURIComponent(id)}`, { cache: "no-store" });
      if (!response.ok) throw new Error("LOAD");
      const body = await response.json(); setSelected(storedSecurityReviewSchema.parse({ record: body.record, etag: body.etag })); setReason(""); setAudit(null);
    } catch { setError("reviewError_LOAD"); }
    finally { busy.current = false; setPending(false); }
  }
  function download() {
    if (!selected) return;
  ...
  return <>
    ...
    <section className="security-review-layout">
      <aside className="security-review-list"><div className="section-title"><h2>{t("reviewHistory")}</h2><button className="refresh-button" title={t("reviewRefresh")} aria-label={t("reviewRefresh")} disabled={pending} onClick={() => void load().catch(() => setError("reviewError_LOAD"))}><RefreshCw size={16} /></button></div>{records.map((entry) => <button type="button" key={entry.record.id} className={entry.record.id === selected?.record.id ? "selected" : ""} disabled={pending} onClick={() => void open(entry.record.id)}><strong>{t(`reviewCase_${entry.record.caseId}`)} · {t(`reviewStatus_${entry.record.status}`)}</strong><span>{entry.record.id}</span><time dateTime={entry.record.createdAt}>{new Date(entry.record.createdAt).toLocaleString(locale)}</time></button>)}{!records.length && !loading && !error && <p>{t("reviewEmpty")}</p>}{cursor && <button className="refresh-button" disabled={pending} onClick={() => void load(cursor).catch(() => setError("reviewError_LOAD"))}>{t("reviewMore")}</button>}</aside>
      <div className="security-review-detail">{selected ? <>
        ...
        <h3>{t("reviewDecision")}</h3>{selected.record.status === "awaiting_decision" ? <div className="security-review-decision"><label htmlFor="security-reason">{t("reviewReason")}</label><textarea id="security-reason" value={reason} maxLength={1000} disabled={pending} onChange={(event) => setReason(event.target.value)} /><div className="security-review-command">{([{ action: "approve", label: "reviewApprove", icon: Check }, { action: "reject", label: "reviewReject", icon: X }, { action: "request_information", label: "reviewRequestInformation", icon: FileSearch }] as const).map(({ action: decision, label, icon: Icon }) => <button className="refresh-button" key={decision} disabled={pending || reason.trim().length < 3 || decision === "approve" && !selected.record.evaluation.controlsPassed} onClick={() => void action({ action: decision, id: selected.record.id, etag: selected.etag, reason })}><Icon size={16} />{t(label)}</button>)}</div></div> : <p>{t("reviewDecisionRecorded")}</p>}
